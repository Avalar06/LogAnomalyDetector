...
def normalize_dataframe(df_raw: pd.DataFrame, source_tag: str) -> pd.DataFrame:
    ...
    # -------------------------------
    # 🔧 2. FIX PROBABILITY CONTRADICTION
    # -------------------------------
    if "prob_anomaly" not in df.columns:
        # Default to 0.95 ONLY if it is an anomaly, otherwise 0.0
    ...
    # -------------------------------
    # 🔧 3. STRICT UTC FORMATTING (Aligns with _read_anomalies)
    # -------------------------------
    def to_strict_utc(x):
        try:
